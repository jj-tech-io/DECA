...
def main(args):
    ...
    #image path C:\Users\joeli\Dropbox\Code\Python Projects\DECA\TestSamples\teaser\0.jpg
    image_path = 'C:\\Users\\joeli\\Dropbox\\Code\\Python Projects\\DECA\\TestSamples\\teaser\\0.jpg'
    testdata = datasets.TestData(image_path, iscrop=args.iscrop, face_detector=args.detector)
    # DECA
    ...
    for i in range(len(testdata)):
        ...
        # print(f"code dict: {codedict}")
        # print(f"visdict: {visdict}")
        ...
        for k in yaw_list: #jaw angle from -50 to 50
            ...
            global_pose = batch_euler2axis(deg2rad(euler_pose[:,:3].cuda())) # global pose is in axis-angle format ie. 3D rotation vector
        ...
        for (i,k) in enumerate(range(len(expdata))): #jaw angle from -50 to 50        
            ...
            print(f"number of expressions: {len(exp_codedict['exp'])}")
    ...
    print(f'visdict_list_list: {visdict_list_list}')
    ### write gif
    writer = imageio.get_writer(os.path.join(savefolder, 'teaser2.gif'), mode='I')
